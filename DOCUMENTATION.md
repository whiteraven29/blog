...
# Create superuser (or use the seeded admin/admin123)
python manage.py createsuperuser

# Seed sample posts and categories (optional)
...
The Axios instance automatically attaches the `Authorization: Bearer <token>` header to every request and silently refreshes the access token on `401` responses.
...
### Backend (example: Ubuntu + Gunicorn + Nginx)
...
# Add your production frontend origin to CORS_ALLOWED_ORIGINS
...
# Serve dist/ via Nginx or any static host (Netlify, Vercel, Cloudflare Pages)
# Update the API base URL in src/api/client.js before building:
# const BASE = 'https://api.yourdomain.com/api'
```
...
Created by `seed_posts.py`. **Change these before deploying.**
